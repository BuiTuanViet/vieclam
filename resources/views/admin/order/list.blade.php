                        ...
                        <table id="example1" class="table table-bordered table-striped">
                            ...
                            <?php $sumPrice = 0;?>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <div class="price">
                                            ...
                                            @if (!empty($orderItem->discount))
                                                ...
                                                <span class="priceDiscount">{{ number_format($orderItem->discount, 0, ',', '.') }}   {{ $orderItem->currency }}</span>
                                                <input type="hidden" class="unitPrice" value="{{ $orderItem->discount }}">
                                            @else
                                                <input type="hidden" class="unitPrice" value="{{ $orderItem->price }}">
                                                <span class="priceDiscount">{{ number_format($orderItem->price, 0, ',', '.') }}   {{ $orderItem->currency }}</span>
                                            @endif
                                        </div>
                                    </td>
                                    <td>
                                    ...
                                    <td>
                                        ...
                                        {{ !empty($orderItem->discount) ? number_format(($orderItem->discount*$orderItem->quantity), 0, ',', '.') : number_format(($orderItem->price*$orderItem->quantity), 0, ',', '.') }}   {{ $orderItem->currency }}
                                    </td>
                                </tr>
                                @endforeach
                                <tr>
                                    <td colspan="4">Tổng tiền</td>
                                    <td>{{ number_format($sumPrice, 0, ',', '.') }}  {{ $orderItem->currency }}</td>
                                </tr>
                            </tbody>
                            ...
                            <tr>
                                <td colspan="2">
                                    ...
                                    <p>Tên: {{ $order->shipping_name }}</p>
                                    <p>Địa chỉ: {{ $order->shipping_address }} - {{ $order->shipping_city }} - {{ $order->ship_country}}</p>
                                ...
                                <td>
                                    <p>-{{ number_format($order->cost_sale, 0, ',', '.') }}   {{ $orderItem->currency }}</p>
                                    <p>-{{ number_format($order->cost_point, 0, ',', '.') }}   {{ $orderItem->currency }}</p>
                                    <p>+{{ number_format($order->cost_ship, 0, ',', '.') }}   {{ $orderItem->currency }}</p>
                                    <p>{{ number_format($order->total_price, 0, ',', '.') }}   {{ $orderItem->currency }}</p>
                                </td>
                            </tr>
                        </table>
                    </div>
